...
echo -e "${NC}"
echo -e "${YELLOW}Welcome to the Automated & Integrated Installation System!${NC}\n"

# ==========================================
# Get User Input Configuration
# ==========================================
echo -e "${CYAN}Please enter the following configuration details:${NC}"
read -p "🔑 Telegram Bot Token: " INPUT_BOT_TOKEN
read -p "🌐 Desired port for Web Panel (e.g., 8080): " INPUT_PANEL_PORT
read -p "👤 Admin Username for Panel login: " INPUT_ADMIN_USER
read -p "🔒 Admin Password for Panel login: " INPUT_ADMIN_PASS

echo -e "\n${GREEN}[1/5] Updating server and installing prerequisites...${NC}"
apt update -q
apt install -y python3-venv python3-pip curl wget sqlite3 net-tools -q
bash -c "$(curl -L https://github.com/XTLS/Xray-install/raw/main/install-release.sh)" @ install

echo -e "\n${GREEN}[2/5] Creating project structure in /root/Proxy-Manager...${NC}"
...
# Save Login Credentials for HTML
echo "{\"username\": \"$INPUT_ADMIN_USER\", \"password\": \"$INPUT_ADMIN_PASS\"}" > /root/Proxy-Manager/admin_auth.json

echo -e "\n${GREEN}[3/5] Writing system core files (main.py, bot.py, panel.html)...${NC}"

# -----------------------------------
# Create main.py (Untouched)
# -----------------------------------
...
echo -e "\n${GREEN}[4/5] Building the CLI management tool (manager)...${NC}"
...
clear
echo -e "${CYAN}==========================================="
echo -e "       ⚙️  Proxy Server Management Menu  ⚙️"
echo -e "===========================================${NC}"
echo "1) 🤖 Change Telegram Bot Token"
echo "2) 🌐 Change Web Panel Port"
echo "3) 🔑 Change Panel Admin/Password"
echo "4) 🔄 Restart Services (Panel & Bot)"
echo "5) 🔴 Uninstall & Completely Clean Server"
echo "0) ❌ Exit"
echo -e "${CYAN}===========================================${NC}"
...
case $choice in
    1)
        ...
        echo -e "${GREEN}✅ Token successfully updated and Bot restarted.${NC}"
        ;;
    2)
        read -p "Enter new port: " new_port
        sed -i -E "s/port=[0-9]+/port=$new_port/g" /root/Proxy-Manager/main.py
        systemctl restart proxy-panel
        echo -e "${GREEN}✅ Port successfully changed to $new_port and Panel restarted.${NC}"
        ;;
    3)
        read -p "New Admin Username: " new_u
        read -p "New Admin Password: " new_p
        echo "{\"username\": \"$new_u\", \"password\": \"$new_p\"}" > /root/Proxy-Manager/admin_auth.json
        echo -e "${GREEN}✅ Panel login credentials successfully updated.${NC}"
        ;;
    4)
        systemctl restart proxy-panel
        systemctl restart proxy-bot
        echo -e "${GREEN}✅ Services successfully restarted.${NC}"
        ;;
    5)
        echo -e "${RED}⚠️ Cleaning up the entire system...${NC}"
        ...
        echo -e "${GREEN}✅ All files and services successfully removed from the server.${NC}"
        ;;
    0)
        exit 0
        ;;
...
echo -e "🌐 Web Panel URL: http://${SERVER_IP}:${INPUT_PANEL_PORT}"
echo -e "🔑 Username: ${INPUT_ADMIN_USER}"
echo -e "🔒 Password: ${INPUT_ADMIN_PASS}"
echo -e "\n💡 For quick management in the future, simply type ${YELLOW}manager${NC} in the terminal."
echo -e "${CYAN}=====================================================${NC}"
