...
ask_input "🤖 توکن ربات تلگرام را وارد کنید" "BOT_TOKEN" "" "true" "true" "^[0-9]+:[A-Za-z0-9_-]+$" "از @BotFather دریافت کنید"
...
echo -e "${CLR_BLUE}  ⬇️  در حال دانلود فایل‌های پروژه از گیت‌هاب...${CLR_RESET}"
REPO_BASE="https://raw.githubusercontent.com/Mnidri/Proxy-Manager/main"
if [ ! -f "bot.py" ]; then
    curl -fsSL "${REPO_BASE}/bot.py" -o bot.py >/dev/null 2>&1 && echo -e "${CLR_GREEN}  ✓${CLR_RESET} bot.py دانلود شد" || echo -e "${CLR_RED}  ✗ bot.py دانلود نشد${CLR_RESET}"
fi
if [ ! -f "main.py" ]; then
    curl -fsSL "${REPO_BASE}/main.py" -o main.py >/dev/null 2>&1 && echo -e "${CLR_GREEN}  ✓${CLR_RESET} main.py دانلود شد" || echo -e "${CLR_RED}  ✗ main.py دانلود نشد${CLR_RESET}"
fi
if [ ! -f "bot.py" ] || [ ! -f "main.py" ]; then
    echo -e "${CLR_RED}❌ فایل‌های اصلی پروژه دانلود نشدند. لطفاً دسترسی اینترنت را بررسی کنید.${CLR_RESET}"
    exit 1
fi
